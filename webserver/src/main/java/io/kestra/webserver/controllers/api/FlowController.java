...
public class FlowController {
        ...
        public FlowGraph flowGraph(
                ...
                String flowUid = revision.isEmpty()
                                ? Flow.uidWithoutRevision(tenantService.resolveTenant(), namespace, id)
        ...
        public Object index(
                ...
                return source ? flowRepository
                                .findByIdWithSource(tenantService.resolveTenant(), namespace, id,
        ...
        @Hidden // we hide it otherwise this is the one that will be included in the OpenAPI
                // spec instead of the YAML one.
        ...
        protected FlowWithSource doCreate(Flow flow, String source) {
                return flowRepository.create(flow, source,
                                pluginDefaultService.injectDefaults(flow.withSource(source)));
        }
        ...
        public List<FlowWithSource> updateNamespace(
                ...
                return this.bulkUpdateOrCreate(
                                ...
                                sources
                                                .stream()
                                                .map(flow -> FlowWithSource.of(yamlParser.parse(flow, Flow.class),
                                                                flow.trim()))
                                                .toList(),
                                delete);
        }
        ...
        @Operation(tags = {
                        "Flows" }, summary = "Update a complete namespace from json object", description = "All flow will be created / updated for this namespace.\n"
                                        +
                                        "Flow that already created but not in `flows` will be deleted if the query delete is `true`", deprecated = true)
        @Deprecated(forRemoval = true, since = "0.18")
        @Hidden // we hide it otherwise this is the one that will be included in the OpenAPI
                // spec instead of the YAML one.
        public List<Flow> updateNamespace(
                ...
                return this
                                .bulkUpdateOrCreate(
                                                ...
                                                flows
                                                                .stream()
                                                                .map(throwFunction(flow -> FlowWithSource.of(flow,
                                                                                flow.generateSource())))
                                                                .toList(),
                                                delete)
                                .stream()
        ...
        protected List<FlowWithSource> bulkUpdateOrCreate(@Nullable String namespace, List<FlowWithSource> flows,
                        Boolean delete) {
                ...
                List<FlowWithSource> updatedOrCreated = flows.stream()
                                .map(flowWithSource -> {
                                        Optional<Flow> existingFlow = flowRepository.findById(
                                                        tenantService.resolveTenant(),
                                                        flowWithSource.getNamespace(), flowWithSource.getId());
                                        if (existingFlow.isPresent()) {
                                                return flowRepository.update(flowWithSource, existingFlow.get(),
                                                                flowWithSource.getSource(),
                                                                pluginDefaultService.injectDefaults(flowWithSource));
                                        } else {
        ...
        protected FlowWithSource update(Flow current, Flow previous, String source) {
                return flowRepository.update(current, previous, source,
                                pluginDefaultService.injectDefaults(current.withSource(source)));
        }

        @ExecuteOn(TaskExecutors.IO)
        @Post(uri = "bulk", consumes = MediaType.APPLICATION_YAML)
        @Operation(tags = {
                        "Flows" }, summary = "Update from multiples yaml sources", description = "All flow will be created / updated for this namespace.\n"
                                        +
                                        "Flow that already created but not in `flows` will be deleted if the query delete is `true`")
        ...
        public HttpResponse<Flow> updateTask(
                        ...
                        throws ConstraintViolationException {
                log.warn(
                                "This endpoint is deprecated: updating a single task is not longer supported and will be removed in a future release.");
                ...
                try {
                        ...
                        return HttpResponse.ok(flowRepository.update(newValue, flow, newSource,
                                        pluginDefaultService.injectDefaults(newValue.withSource(newSource))).toFlow());
                } catch (InternalException e) {
                        return HttpResponse.status(HttpStatus.NOT_FOUND);
                }
        }
        ...
        public HttpResponse<Void> delete(
                        @Parameter(description = "The flow namespace") @PathVariable String namespace,
                        @Parameter(description = "The flow id") @PathVariable String id) {
                Optional<FlowWithSource> flow = flowRepository.findByIdWithSource(tenantService.resolveTenant(),
                                namespace, id);
                if (flow.isPresent()) {
        ...
        public FlowTopologyGraph dependencies(
                ...
                List<FlowTopology> flowTopologies = flowTopologyRepository.findByFlow(tenantService.resolveTenant(),
                                namespace,
                                id, destinationOnly);
                                        ...
                                        try {
                                                ...
                                                if (sentRevision != null) {
                                                        Integer lastRevision = Optional
                                                                        .ofNullable(flowRepository.lastRevision(
                                                                                        tenantService.resolveTenant(),
                                                                                        flowParse.getNamespace(),
                                                                                        flowParse.getId()))
                                                                        .orElse(0);
                                                        validateConstraintViolationBuilder.outdated(
                                                                        !sentRevision.equals(lastRevision + 1));
                                                }

                                                validateConstraintViolationBuilder.deprecationPaths(
                                                                flowService.deprecationPaths(flowParse));
                                                validateConstraintViolationBuilder
                                                                .warnings(flowService.warnings(flowParse));
                                                ...
                                                validateConstraintViolationBuilder.infos(Stream
                                                                .concat(relocationsList.stream(),
                                                                                flowService.missingDefaults(flowParse)
                                                                                                .stream())
                                                                .toList());
                                                ...
                                                modelValidator.validate(pluginDefaultService
                                                                .injectDefaults(flowParse.withSource(flow)));
                                                flowService.checkValidSubflows(flowParse,
                                                                tenantService.resolveTenant());
                                        } catch (ConstraintViolationException e) {
                                                validateConstraintViolationBuilder.constraints(e.getMessage());
                                        } catch (RuntimeException re) {
                                                // In case of any error, we add a validation violation so the error is
                                                // displayed
                                                // in the UI.
                                                // We may change that by throwing an internal error and handle it in the
                                                // UI, but
                                                ...
                                                validateConstraintViolationBuilder
                                                                .constraints("Unable to validate the flow: "
                                                                                + re.getMessage());
                                        }
        ...
        // This endpoint is not used by the Kestra UI nor our CLI but is provided for
        // the API users for convenience
        @ExecuteOn(TaskExecutors.IO)
        ...
        public ValidateConstraintViolation validateTask(
                ...
                try {
                        ...
                        validateConstraintViolationBuilder
                                        .constraints("Unable to validate the task: " + re.getMessage());
                }
        ...
        public ValidateConstraintViolation validateTrigger(
                ...
                try {
                        ...
                        validateConstraintViolationBuilder
                                        .constraints("Unable to validate the trigger: " + re.getMessage());
                }
                return validateConstraintViolationBuilder.build();
        }
        ...
        public HttpResponse<byte[]> exportByQuery(
                        ...
                        throws IOException {
                var flows = flowRepository.findWithSource(query, tenantService.resolveTenant(), scope, namespace,
        ...
        public HttpResponse<byte[]> exportByIds(
                ...
                var flows = ids.stream()
                                .map(id -> flowRepository
                                                .findByIdWithSource(tenantService.resolveTenant(), id.getNamespace(),
                                                                id.getId())
                                                .orElseThrow())
                                .toList();
                var bytes = HasSource.asZipFile(flows, flow -> flow.getNamespace() + "." + flow.getId() + ".yml");
        ...
        public HttpResponse<BulkResponse> deleteByQuery(
                ...
                List<Flow> list = flowRepository
                                .findWithSource(query, tenantService.resolveTenant(), scope, namespace,
                                                RequestUtils.toMap(labels))
                                .stream()
        ...
        public HttpResponse<BulkResponse> disableByQuery(
                ...
                return HttpResponse.ok(BulkResponse.builder()
                                .count(setFlowsDisableByQuery(query, scope, namespace, labels, true).size()).build());
        }
        ...
        public HttpResponse<BulkResponse> enableByQuery(
                ...
                return HttpResponse.ok(BulkResponse.builder()
                                .count(setFlowsDisableByQuery(query, scope, namespace, labels, false).size()).build());
        }
        ...
        protected List<FlowWithSource> setFlowsDisableByQuery(String query, List<FlowScope> scope, String namespace,
                        List<String> labels, boolean disable) {
                return flowRepository
                                .findWithSource(query, tenantService.resolveTenant(), scope, namespace,
                                                RequestUtils.toMap(labels))
                                .stream()
        ...
        protected <T> T parseTaskTrigger(String input, Class<T> cls) throws ConstraintViolationException {
                try {
                        ...
                        if (e.getCause() instanceof ConstraintViolationException constraintViolationException) {
                                ...
                                throw new ConstraintViolationException(
                                                ...
                                                Set.of(
                                                                ManualConstraintViolation.of(
                                                                                "Invalid type: " + invalidTypeIdException
                                                                                                .getTypeId(),
                                                                                input,
                                                                                String.class,
                                ...
                                var message = unrecognizedPropertyException.getOriginalMessage()
                                                + unrecognizedPropertyException.getMessageSuffix();
                                throw new ConstraintViolationException(
                                                message,
                                                Collections.singleton(
                                                                ManualConstraintViolation.of(
                                                                                e.getCause() == null ? message
                                                                                                : message + "\nCaused by: "
